 ...
 */
export function shouldIncludeCommit(files, targetDir, excludeDir) {
 ...
 */
export async function filterCommitsByDirectory(octokit, commits, owner, repo, targetDir, excludeDir) {
  if (!targetDir && !excludeDir) {
    return commits;
  }
 ...
 * @param {string} options.excludeDir - Directory to exclude
...
export async function getCommitsBetween(options) {
  ...
  try {
    ...
    if (options.targetDir || options.excludeDir) {
      commits = await filterCommitsByDirectory(octokit, commits, owner, repo, options.targetDir, options.excludeDir);
    }
