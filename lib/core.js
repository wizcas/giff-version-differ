...
const REQUEST_SIZE = 100; // Increased from 20 for better performance
...
export async function getCommitsBetween(options) {
  ...
  try {
    ...
    if (options.restOnly) {
      ...
      try {
        commits = await getCommitsBetweenGraphQL(graphqlWithAuth, owner, repo, fromSha, toSha, options.maxCommits);
        apiUsed = "graphql";
      } catch (error) {
...
export async function streamCommitsBetween(options, onCommitBatch, onProgress) {
  ...
  try {
    ...
    try {
      ...
      onProgress("GraphQL failed, falling back to REST API...");
      commits = await getCommitsBetweenREST(octokit, owner, repo, fromSha, toSha);
      apiUsed = "REST-fallback";
    }
