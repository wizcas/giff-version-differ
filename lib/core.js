...
export async function getCommitList(graphqlWithAuth, owner, repo, baseSha, headSha) {
  ...
  try {
    ...
    if (!result.repository || !result.repository.object) {
      throw new Error(`Could not find commit ${headSha} in repository ${owner}/${repo}`);
    }

    const allCommits = result.repository.object.history.nodes;

    // Filter commits to only include those between baseSha and headSha
    const commits = [];
    for (const commit of allCommits) {
      commits.push(commit);
      // Stop when we reach the base commit (compare first 7 characters)
      if (commit.oid.startsWith(baseSha.substring(0, 7))) {
        break;
      }
    }
    ...
    // Re-throw to trigger fallback to REST API
    throw error;
  }
}
 ...
 * @param {string} baseSha - Base commit SHA (older)
 * @param {string} headSha - Head commit SHA (newer)
 * @param {number} maxCommits - Maximum number of commits to fetch (default: 10000)
 * @returns {object} - Object containing commits array and fetch statistics
 */
export async function getCommitsBetweenGraphQL(graphqlWithAuth, owner, repo, baseSha, headSha, maxCommits = 10000) {
  try {
    const startTime = Date.now();
    console.log(`[GraphQL] Starting commit fetch from ${headSha.substring(0, 7)} to ${baseSha.substring(0, 7)}, maxCommits: ${maxCommits}`);
    ...
    while (hasNextPage && !foundBase && allCommits.length < maxCommits) {
      requestCount++;
      ...
      console.log(
        `[GraphQL] Request ${requestCount}: Fetching ${REQUEST_SIZE} commits ${cursor ? `after cursor ${cursor}...` : "from HEAD"}`
      );

      const result = await graphqlWithAuth(paginatedQuery, variables);
      const commits = result.repository.object.history.nodes;

      console.log(`[GraphQL] Request ${requestCount}: Received ${commits.length} commits`);

      for (const commit of commits) {
        totalChecked++;

        // Check if we found the base commit (exact match)
        if (commit.oid === baseSha) {
          foundBase = true;
          console.log(
            `[GraphQL] Found base commit ${baseSha.substring(0, 7)} after checking ${totalChecked} commits in ${requestCount} requests`
          );
          break;
        }
      ...
      // Add early termination if we're checking too many commits
      if (totalChecked >= maxCommits && !foundBase) {
        console.warn(`[GraphQL] Reached maxCommits limit (${maxCommits}) without finding base commit ${baseSha.substring(0, 7)}`);
        break;
      }
    }
    ...
    const stats = {
      ...
      elapsedTime,
      foundBase,
      returnedCommits: allCommits.length,
    };

    if (!foundBase) {
      console.warn(
        `[GraphQL] Base commit ${baseSha.substring(
          0,
          7
        )} not found after checking ${totalChecked} commits in ${requestCount} requests (${elapsedTime}ms)`
      );
    } else {
      console.log(
        `[GraphQL] Successfully found ${allCommits.length} commits between ${headSha.substring(0, 7)} and ${baseSha.substring(
          0,
          7
        )} after checking ${totalChecked} total commits in ${requestCount} requests (${elapsedTime}ms)`
      );
    }

    return { commits: allCommits, stats };
  } catch (error) {
 ...
 * @returns {Array} - Array of commit objects
 */
export async function getCommitsBetweenREST(octokit, owner, repo, baseSha, headSha, targetDir = null) {
...
export async function getCommitsBetween(options) {
  ...
  try {
    ...
    if (options.restOnly) {
      commits = await getCommitsBetweenREST(octokit, owner, repo, fromSha, toSha, options.targetDir);
      apiUsed = "rest";
    } else {
      try {
        const result = await getCommitsBetweenGraphQL(graphqlWithAuth, owner, repo, fromSha, toSha, options.maxCommits);
        commits = result.commits;
        apiUsed = "graphql";
        console.log(`[Core] GraphQL stats: checked ${result.stats.totalChecked} commits in ${result.stats.requestCount} requests`);
      } catch (error) {
...
export async function streamCommitsBetween(options, onCommitBatch, onProgress) {
  ...
  try {
    ...
    if (options.restOnly) {
      safeProgress("Using REST API (forced)...");
      ...
      try {
        ...
        const result = await getCommitsBetweenGraphQL(graphqlWithAuth, owner, repo, fromSha, toSha, options.maxCommits);
        commits = result.commits;
        fetchStats = { ...result.stats, elapsedTime: Date.now() - fetchStartTime };
        apiUsed = "GraphQL";

        safeProgress(
          `GraphQL fetch completed: ${fetchStats.totalChecked} commits checked in ${fetchStats.requestCount} requests (${formatElapsedTime(
            fetchStats.elapsedTime
          )})`
        );
      } catch (error) {
        console.warn(`GraphQL failed, falling back to REST API: ${error.message}`);
        safeProgress("GraphQL failed, falling back to REST API...");
        const fetchStartTime = Date.now();
        commits = await getCommitsBetweenREST(octokit, owner, repo, fromSha, toSha, options.targetDir);
        fetchStats.elapsedTime = Date.now() - fetchStartTime;
        apiUsed = "REST-fallback";
